...
| `--disable-workarounds`   | enabled                                                        | MQTT `home_flag` / `ipcam.file` rewrites; in-print cover (`get_subtask_info` + `cover_cache`); `start_sdcard_print` LAN MQTT instead of signed cloud REST. Does **not** disable `ft_*`/`:6000`, native file-browser passthrough, RTSPS, firmware info, or preset sync. Maps to `-DOBN_ENABLE_WORKAROUNDS=OFF`. |
