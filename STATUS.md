...
| `bambu_network_get_user_tasks` | ✅ | `GET /v1/user-service/my/tasks?limit=&offset=&status=[&deviceId=]` — response `{total,hits:[…]}` forwarded verbatim to Studio's TaskManager / Print History WebView. Confirmed against MITM of stock `02.08.01.51`. Empty envelope under `block_cloud`. |
...
| Function | Status | Notes |
| --- | :--: | --- |
| `bambu_network_start_print` | ⚠️ | **Cloud print (`use_lan_channel=false`).** Config 3mf → S3 (history/preview) + main `.3mf` → S3 + project PATCH with S3 URL + `POST /my/task` `mode=cloud_file`. Cloud dispatches the print; OBN **does not** publish MQTT `project_file` (would double-fire / `0500_4004`). Prerequisites: **`slicer_key.pem`** + **`client_name = BambuStudio`**. See [NETWORK_PLUGIN.md §6.8.1.1](NETWORK_PLUGIN.md#6811-cloud-print-authorization-obn-findings-on-hardware-2026-07). |
| `bambu_network_start_local_print_with_record` | ⚠️ | **LAN delivery (`use_lan_channel=true`) — intentional divergence from stock.** Config 3mf → S3 (history only); **main** `.3mf` via **FTPS** (`PrintParams.ftp_folder`, no BRTC); one project PATCH with `ftp://…`; `POST /my/task` `mode=lan_file` (cloud dispatch). No plugin MQTT `project_file`, no main S3 upload. On missing IP/code or FTPS failure returns `< 0` so Studio can fall back to `start_print`. Stock still dual-uploads and overwrites the project URL with S3 ([NETWORK_PLUGIN.md §6.8.1.2](NETWORK_PLUGIN.md#6812-stock-print-transports-ftps-vs-brtc-wire-confirmed-p2s-2026-07)). |
...
| --- | --- | --- | --- |
| `bambu_network_start_print` | `run_cloud_print_job(..., use_lan_channel=false)` | `cloud_print.cpp` | Config→S3 + main→S3 + PATCH S3 URL + `/my/task` `cloud_file` (cloud dispatch; no MQTT `project_file`) |
| `bambu_network_start_local_print_with_record` | `run_cloud_print_job(..., use_lan_channel=true)` | `cloud_print.cpp` | Config→S3 + main→**FTPS** + PATCH `ftp://` + `/my/task` `lan_file` (cloud dispatch; no MQTT; no main S3). Fail → Studio `start_print` fallback |
