...
std::string Agent::camera_ticket_url_for(const std::string& dev_id)
{
    ...
    // The cloud ttcode is the sole authorization the printer needs: presenting
    // it over the IOTC/AV session opens the printer's camera listener. No LAN
    // arming command is sent — the cloud ttcode alone is what the genuine flow
    // relies on to start streaming.
