...
//
// All knowledge here is derived from static analysis (nm, objdump, strings) of
// libBambuSource.so (ELF x86-64, version 4.3.3.4-0-gf6d6219_openssl_x64)
// which embeds the TUTK IOTC SDK.  No disassembled code from that binary is
// reproduced here; only the protocol structures and observed constants are
// documented.
//
// ==========================================================================
...
//
// Connection phases (observed from log strings + disassembly):
//
//   1. INITIALIZATION
...
//         UID validation: exactly 20 printable ASCII characters (verified by
//         IsUIDVaild: loops 0x14 iterations, tests ctype isprint bit 0x08).
...
// This identifies the TUTK IOTC SDK version embedded in libBambuSource.so.
//
// ==========================================================================
...
// --------------------------------------------------------------------------
// IOTC session limits (from disassembly)
// --------------------------------------------------------------------------
...
// documentation and community reverse engineering puts it at port 443 or
...
// --------------------------------------------------------------------------
//
// Observed from _IOTC_Send_Search_R disassembly (0x18dc90):
//
...
// From TutkBambuSource.cpp IotcConnectCfg and Ghidra notes:
...
// From TutkBambuSource.cpp and avClientStartEx disassembly (0x160790).
...
//
// NOTE on channel: The BambuTunnelTutk::open Ghidra notes say 0x38 but the
// avClientStartEx validator (0x1607f6) does cmpb $0x1f, 0x8(%rdi) and takes
...
// From avClientStartEx disassembly (writes back fields at 0x160883–0x1608a7):
