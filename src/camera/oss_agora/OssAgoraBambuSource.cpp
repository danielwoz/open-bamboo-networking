...
int OssAgoraBambuSource::bambu_create(void** out_tunnel, const std::string& url)
{
    ...
    if (!AgoraUrl::parse(url, t->url, err)) {
        std::fprintf(stderr, "[oss-agora-src] bad URL: %s\n", err.c_str());
        delete t;
        return -1;
    }
...
int OssAgoraBambuSource::bambu_open(void* tunnel)
{
    ...
    std::fprintf(stderr,
        "[oss-agora-src] open: channel=%s token=%s tutk_uid=%s device=%s region=0x%x\n",
        p.channel.c_str(), p.token.c_str(), p.tutk_uid.c_str(),
