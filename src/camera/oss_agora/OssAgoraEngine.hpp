...
//
// OssAgoraEngine — open-source drop-in replacement for libagora_rtc_sdk.so.
//
// Exports createAgoraRtcEngine() with the same ABI that libBambuSource.so
// expects (Agora SDK 4.x, x86_64 Linux Itanium ABI).
//
// Vtable layout verified by Ghidra analysis of libBambuSource.so
// (not stripped). The engine object is a plain-C struct with a manually
...
//     0x20 → slot  4: initialize(RtcEngineContext*)  CONFIRMED Ghidra
//     0xC8 → slot 25: (setup call, no args)          CONFIRMED Ghidra initAgora
//     0x150→ slot 42: leaveChannel()
//     0x158→ slot 43: (cleanup call, no args)        CONFIRMED Ghidra initAgora
//     0x1A8→ slot 53: (enable call, arg=0)           CONFIRMED Ghidra initAgora
//     0x1B0→ slot 54: (enable call, arg=1)           CONFIRMED Ghidra initAgora
//     0x7E0→ slot252: joinChannelWithOptions(token,ch,info,uid,opts,evtHdlr)
//                                                    CONFIRMED Ghidra joinChannel
//     0x8B0→ slot278: enableEncryption(ch,key,mode)  CONFIRMED Ghidra joinChannel
//
