...
//
// OssAgoraEngine.cpp — drop-in replacement for libagora_rtc_sdk.so.
//
// Protocol notes (derived by reverse-engineering libBambuSource.so with
// Ghidra 11.4.3; no code copied — only call sites and vtable offsets were
// read):
//
//   libBambuSource.so's StaticAgoraLib calls through the IRtcEngine vtable
//   at the exact byte offsets listed in OssAgoraEngine.hpp. Every slot that
//   libBambuSource actually exercises must exist in our vtable; unknown slots
//   default to oss_noop (returns 0) so forward-compat stubs don't crash.
//
//   The joinChannel call (slot 252, 0x7E0) passes:
//     (engine*, token, channel_or_uid_struct*, options_struct*, event_handler*)
//   We extract channel+uid from the struct passed in 'ch_struct'.
//
//   enableEncryption (slot 278, 0x8B0):
//     (engine*, ch_name, key_material*, mode_int)
//   Recorded for later use once the signaling layer is implemented.
//
//   queryInterface(4, &out) → IMediaEngine shim
//   IMediaEngine::registerVideoFrameObserver(3) → store observer shim ptr
//
// TODO: Connect to Agora SD-RTN signaling (ap.agora.io → edge server →
//       VOCS binary protocol → ICE credentials → DTLS/SRTP media).
//       See OssAgoraSignaling.cpp (skeleton).
...
static int media_registerVideoFrameObserver(OssMediaEngine* self, void* observer)
{
    ...
    std::fprintf(stderr,
        "[oss-agora] registerVideoFrameObserver(%p)\n", observer);
    return 0;
}
...
static int eng_initialize(OssEngine* self, const void* ctx_raw)
{
    ...
    std::fprintf(stderr, "[oss-agora] initialize(app_id=%s)\n",
                 self->app_id.c_str());

    self->edge_servers = agora_discover_edge_servers(self->app_id, self->area_code);
    std::fprintf(stderr, "[oss-agora] initialize: %zu edge server(s) discovered\n",
...
static void eng_release(OssEngine* self, bool /*sync*/)
{
    std::fprintf(stderr, "[oss-agora] release()\n");
    self->released = true;
    if (self->signaling) {
                            ...
                            void*       /*event_handler*/)
{
    ...
    std::fprintf(stderr, "[oss-agora] joinChannel(channel=%s uid=%u)\n",
    ...
    if (rc != 0) {
        std::fprintf(stderr, "[oss-agora] joinChannel: signaling join failed\n");
...
{
    self->joined.store(false);
    std::fprintf(stderr, "[oss-agora] leaveChannel()\n");
    if (self->signaling) {
        self->signaling->leave();
    }
    return 0;
...
static int eng_enableEncryption(OssEngine* self,
                                 ...
                                 const void* /*config*/)
{
    std::fprintf(stderr, "[oss-agora] enableEncryption(enabled=%d)\n",
                 (int)enabled);
    return 0;
}
...
static void init_engine_vtable()
{
    ...
    kEngineVtable[4]   = (void*)eng_initialize;                 // initialize ← Ghidra 0x20
    ...
    kEngineVtable[252] = (void*)eng_joinChannel;                // joinChannel ← Ghidra 0x7E0
    kEngineVtable[278] = (void*)eng_enableEncryption;           // enableEncryption ← Ghidra 0x8B0
}

static bool g_vtable_inited = false;

extern "C" void* createAgoraRtcEngine()
{
    if (!g_vtable_inited) {
        init_engine_vtable();
        g_vtable_inited = true;
    }
    auto* e = new OssEngine();
    e->vtable      = kEngineVtable;
    e->frame_queue = nullptr;
    std::fprintf(stderr, "[oss-agora] createAgoraRtcEngine() → %p\n", e);
    return e;
}
